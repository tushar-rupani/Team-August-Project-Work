...
router.get("/home", handleLogin, async (req, res) => {
    ...
    try {
        var result = await connection.execute(qry_show);
        console.log();
    }
    catch (err) {
    ...
    res.render("index", {result: result[0], name: name, attendance: executeAttendance[0], activity: executeActivity, breakAns, hasCheckedIn, breakoutAns, activatePage, checked_out});
})
